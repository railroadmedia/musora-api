...
class ContentController extends Controller
{
    ...
    public function showAllPacks(Request $request)
    {
        ...
        foreach ($allMyPacks as $slug => $pack) {
            ...
            if (array_key_exists($slug, array_flip($productSkuToPackSlugArray))) {
                $sku = array_flip($productSkuToPackSlugArray)[$slug];
                if (array_key_exists($sku, config('products.packs-on-regular-purchase', []))) {
                    $myPacks[$slug]['full_price'] = config('products.packs-on-regular-purchase', [])[$sku]['fullPrice'];
                    $myPacks[$slug]['price'] = config('products.packs-on-regular-purchase', [])[$sku]['price'];
                }
            }
        }

        $topPack = [];
            //$this->getTopHeaderPack($myPacks, $newProducts);
